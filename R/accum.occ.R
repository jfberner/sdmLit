...
#'
#' Function to evaluate Ecological Niche Models by the Accumulation of Occurrences of each algorithm \insertCite{jimenezLeavingAreaReceiving2020a}{sdmLit}.
#'
#' @author Laura Jiménez
#'
#' @param sp.name character chain wiht species name, used for plot legends
#' @param output.mod Output from sdm_to_output.mod(). Alternatively, csv file with the following columns: long, lat, output values from SDM, environmental variables and as much rows as cells in the area of study
#' @param occ.pnts  Output from sdm_to_occ.pnts(). Alternatively, csv file with same colums than 'output.mod' and as much rows as occurrence points.
#' @param null.mod indicate the distribution used as null model, possible values are "binomial" and "hypergeom".
#' @param conlev probability that indicates the confidence level to be drawn around the null model. When 'flag=F', 'conlev' must be equal to 0.
#' @param bios vector with two numbers that indicate which environmental variables to plot, if bios==0, the scatterplot pairs will be drawn
#'
#' @return Data Frame with accumulation of occurrences.
#' @export
#'
#' @import dismo
#' @import fields
